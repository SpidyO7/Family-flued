...
function isWeakAnswerMatch(guessText, answerObj) {
  ...
  if (!guessNorm) return false;
  const candidates = [answerObj.text, ...(answerObj.aliases || [])].map(normalizeAnswer);
  return candidates.some(candidate => phraseContains(guessNorm, candidate));
}
...
function findMatchingAnswerIndex(guessText) {
  ...
  const match = getAnswerMatch(guessText, q.answers, game.revealedIndices);
...
function loadQuestion() {
  ...
  game.turn = "A"; // Team A always answers first each round
...
function updateTurnIndicator() {
  ...
  document.getElementById("control-flag-a").textContent = "TEAM A'S TURN";
  document.getElementById("control-flag-b").textContent = "TEAM B'S TURN";
...
function handleGuessSubmit() {
  ...
  if (idx !== -1) {
    revealAnswerByIndex(idx, answeringTeam);
  } else {
    flashWrongGuess();
    AudioFX.strike();
  }
